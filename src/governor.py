...
class GovernorConfig:
    ...
    def from_env(project_root: Optional[str] = None) -> "GovernorConfig":
        ...
        dry = os.getenv("CLT_E8_DRY_RUN", "0").strip().lower() in ("1","true","yes","on")
...
class Governor:
    ...
    def _norm(self, path: str) -> str:
        # Anchor all relative paths to the project root, not the current working dir
    ...
    def is_write_allowed(self, path: str) -> bool:
        ...
        for sub in self.cfg.write_allow:
            target = self._norm(sub)  # subdir under project root
    ...
    def write_text(self, path: str, text: str, encoding: str = "utf-8", bom: bool = False) -> None:
        apath = self._norm(path)
        if not self.is_write_allowed(apath):
            raise PermissionError(f"Governor blocked write outside allowlist: {path}")
        if self.cfg.dry_run: return
        self.ensure_parent(apath)
        if bom:
            with open(apath, "wb") as f:
                f.write(b"\xEF\xBB\xBF" + text.encode("utf-8"))
            return
        with io.open(apath, "w", encoding=encoding, newline="\n") as f:
            f.write(text)
    ...
    def append_text(self, path: str, text: str, encoding: str = "utf-8") -> None:
        ...
        if self.cfg.dry_run: return
        self.ensure_parent(apath)
        with io.open(apath, "a", encoding=encoding, newline="\n") as f:
            f.write(text)

    def copy_file(self, src: str, dst: str) -> None:
        if not os.path.isfile(src): raise FileNotFoundError(src)
        adst = self._norm(dst)
        if not self.is_write_allowed(adst):
            raise PermissionError(f"Governor blocked copy outside allowlist: {dst}")
        if self.cfg.dry_run: return
        self.ensure_parent(adst)
        with open(src, "rb") as r, open(adst, "wb") as w:
            w.write(r.read())

    def approved_targets(self) -> list[str]:
        return [self._norm(sub) for sub in self.cfg.write_allow]
