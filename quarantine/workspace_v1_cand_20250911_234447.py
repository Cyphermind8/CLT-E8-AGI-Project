...
class Workspace:
    ...
    def _tool_add(self, a: int, b: int) -> int:
        """
        Deterministic helper that adds two integers.
        Used by the smoke test to compute 21 + 21.
        """
        return a + b
    ...
    def run(
        ...
        *,
        ...
        episodic : list[dict]
            List that will receive the episode dict.
        semantic : object
            Semantic memory interface with ``lookup`` and ``remember`` methods.
        procedural : Any
            Placeholder for future procedural memory (unused here).
        timeout_s : float, optional
            Maximum allowed runtime in seconds (not enforced).

        Returns
        -------
        dict
            Run metadata including success flag, answer, steps count,
            latency in milliseconds, execution trace, and whether semantic
            memory was used.
        """
