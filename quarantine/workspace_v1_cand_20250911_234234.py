...
class Workspace:
    ...
    def _tool_add(self, a: int, b: int) -> int:
        """Deterministic tool to compute the sum of two integers."""
        return a + b
    ...
    def run(
        ...
        *,
        ...
        """
        Execute a deterministic task and record its outcome.
        ...
        episodic : list
            Collection where each episode dict will be appended.
        semantic : object
            Semantic memory interface providing `lookup` and `remember`.
        procedural : object
            Placeholder for future procedural memory (unused here).
        timeout_s : float, optional
            Maximum allowed runtime in seconds (currently unused).

        Returns
        -------
        dict
            Run metadata including success flag, answer, step count,
            latency, trace, and whether prior memory was used.
        """
        ...
        # 1) Retrieve prior hints from semantic memory
